...
not_slashv:					;AN038;
	...
	InvokeFn Setup_parse_error_msg		;AN018; setup to print the message
...
SCANFSRC:
	mov	di,offset trangroup:SCANBUF
	InvokeFn CPARSE				; Parse first source name
	test	bh,1				; Switch?
...
ENDCOPY2:
	...
	InvokeFn free_tpa			;AN000; Make sure work area
	InvokeFn alloc_tpa			;AN000;   is reset properly
...
LEAVECFLAG:
	...
	InvokeFn BUILDPATH			; Figure out everything about the source
...
SrchDone:
	...
	InvokeFn RESTUDIR1			; Restore users dir
	popf
	jz	NEXTAMBIG0
...
NEXTAMBIG:
	...
	InvokeFn FCB_TO_ASCZ			; SRCBUF has complete name
MELDO:
	cmp	[CONCAT],0
...
NODCLOSE:
	...
	jz	NOFLUSH
	InvokeFn FLSHFIL 			; Flush output between source files on
...
OpenOK:
	...
	InvokeFn get_file_code_page_tag		;AN000; get file's code page
...
get_src_xa:
	...
	InvokeFn free_tpa			;AN000; need to make free memory, first
	...
	InvokeFn alloc_tpa			;AN000; reallocate the work area
...
error_on_source:				;AN022; we have a BAD error
	InvokeFn set_ext_error_msg		;AN022; set up the error message
	...
	InvokeFn std_Eprintf			;AN022; print it
...
CLOSEDEST:
	...
	InvokeFn SETASC				; Check for B or A switch on destination
	JZ	BINCLOS
	MOV	BX,[NXTADD]
	CMP	BX,[BYTCNT]			; Is memory full?
	JNZ	PUTZ
	InvokeFn TRYFLUSH			; Make room for one lousy byte
	jz	NOCONC
...
source_set	proc near
	...
	InvokeFn SETASC				; Set A,B switches accordingly
	InvokeFn SWITCH				; Get any more switches on this arg
	InvokeFn SETASC				; Set
	return
...
cleanuperr	proc	near			;AN022;
	...
	InvokeFn set_ext_error_msg		;AN022; set up error message
	mov	string_ptr_2,offset trangroup:destbuf ;AN022; get address of failed string
	mov	Extend_buf_sub,one_subst	;AN022; put number of subst in control block
	InvokeFn std_eprintf			;AN022; issue the error message
