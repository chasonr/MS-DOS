...
Procedure   PromptBat,NEAR
	...
	InvokeFn BATOPEN 			; attempt to open batch file
	jc      @F
	    ret
	@@:
...
BatDie:
	...
	POP	DS
	ASSUME	DS:TranGroup
	InvokeFn std_eprintf			;AC022; display message
...
ASKFORBAT:
	...
	InvokeFn std_eprintf			 ;Prompt for batch file on stderr
	mov	dx,offset trangroup:pausemes_ptr ;AN000; get second part of message
	InvokeFn std_eprintf			 ;AN000; print it to stderr
	CALL	GetKeystroke
	POP	DS
	ASSUME	DS:ResGroup
	jmp	PromptBat
...
assume	DS:nothing				;AN022;
	...
	InvokeFn dstrlen 			;AN022; get length of string
...
TESTNOP:
	...
	MOV	DS,AX
	ASSUME	DS:ResGroup
	InvokeFn SkipDelim			; skip to first non-delim
;
...
NOPLINE:
	CALL	SkipToEOL
	InvokeFn GetBatByt			; eat trailing LF
	TEST	[BATCH],-1			; are we done with the batch file?
...
Found_EOL:
	...
	InvokeFn GetBatByt			; Eat linefeed
	InvokeFn BATCLOSE
...
try_nextflag:
	...
	InvokeFn CRLF2				;G  Print out prompt

no_crlf_print:
	InvokeFn PRINT_PROMPT			;G
...
NEEDPARM:
	InvokeFn GetBatByt			; get next character
...
GETENV1:
	InvokeFn GetBatByt			; get the byte
	STOSB					; store it
	CMP	AL,0Dh				; EOL?
...
GETENV2:
	...
	ASSUME DS:TRANGROUP
	PUSH	CX
	InvokeFn FIND_NAME_IN_environment
	ASSUME ES:RESGROUP
	POP	CX
	PUSH	ES
	POP	DS
...
ASSUME	ES:RESGROUP
	...
	InvokeFn IOSET				; Set up any redirection
...
;
EACHPARM:
	InvokeFn SCANOFF 			; skip to argument
;
