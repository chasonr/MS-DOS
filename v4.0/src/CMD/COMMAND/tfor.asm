...
for_begin1:
	...
	InvokeFn argv_calc			; compute argv[x] address
...
no_crlf_pr:
	...
	assume	DS:trangroup
	push	di
	InvokeFn PRINT_PROMPT			;G Prompt the user
	pop	di
...
for_do:
	...
	push	SI
	push	BP
	InvokeFn FREE_TPA			; need to make free memory, first
ASSUME	ES:RESGROUP
	call	ForOff
	mov	BX, SIZE for_info - SIZE arg_unit
	InvokeFn Save_Args			; extra bytes needed for for-info
	pushf
	mov	[ForPtr], AX
	InvokeFn ALLOC_TPA			; ALLOC_TPA clobbers registers...
	popf
	pop	BP
	pop	SI
	pop	DI
...
nextarg:
	...
	InvokeFn argv_calc			; convert array index to pointer
