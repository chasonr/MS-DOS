...
SWLOOP:
	InvokeFn SCANOFF 			; Skip any delimiters
...
append_internal:
	...
	InvokeFn ioset				; re-direct the o'l io
...
;
    ...
    InvokeFn ioset				; re-direct the ol' i/o

dont_set_io:					;AN041;
    InvokeFn SETSTDINON				;AN026; turn on critical error on STDIN
    InvokeFn SETSTDOUTOFF			;AN026; turn off critical error on STDOUT
...
ENDIF
RESEARCH:
	InvokeFn path_search			; find the mother (result in execpath)
...
NeoExecute:
	InvokeFn IOSET
	InvokeFn SETSTDINOFF			;AN026; turn off critical error on STDIN
	InvokeFn SETSTDOUTOFF			;AN026; turn off critical error on STDOUT
	MOV	ES,[TRAN_TPA]
	MOV	AH,DEALLOC
...
KanjiScan:
	LODSB					; get a byte
	InvokeFn TestKanj			; is it a leadin byte
...
cmd_copy  proc near
	...
	MOV	SI,OFFSET TRANGROUP:COMBUF+2
	InvokeFn Scanoff 			; advance past separators...
	add	si,PathPos
	mov	di,81h
	xor	cx,cx
