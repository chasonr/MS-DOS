...
;-------------------------------------------------------------------------------;AN000;
  ...
  jnc @F 			    ; If error when loading messages		;AN000;
  	 			    ; then,					;AN000;
...
;-------------------------------------------------------------------------------;AN000;
    ...
    INT     2FH 		     ;						;AN000;
										;AN000;
   cmp AH,0FFH			     ; IF already installed			;AN000;
   jne else_l_1			     ; then,					;AN000;
   ;----------------------------------------------------------------------------;AN000;
   ...
   jmp endif_l_1
   else_l_1:			    ; ELSE, not installed:			;AN000;
   ;------------------------------------------------------------------------	;AN000;
...
;-------------------------------------------------------------------------------;AN000;
   ...
   cmp AX,OFFSET LIMIT		    ; If there is no bytes available past	;AN000;
   jae else_l_2			    ;	the end of our .COM file		;AN000;
      XOR     AX,AX		    ; then, AX := 0 bytes available		;AN000;
   jmp endif_l_2
   else_l_2:			    ;						;AN000;
      SUB     AX,OFFSET LIMIT	    ; else,  AX := Number of FREE bytes 	;AN000;
   endif_l_2:			    ;	     in this segment			;AN000;
										;AN000;
...
;---than the size of the existing Shared data area.				;AN000;
   cmp INSTALLED,YES		    ; If already installed then,		;AN000;
   jne endif_l_3								;AN000;
     ...
     cmp AX,CX			    ; If AX > size of existing SDA		;AN000;
     jbe @F
...
;-------------------------------------------------------------------------------;AN000;
  ...
  jnc @F 			   ; If error when parsing the command		;AN000;
    				   ; line then, EXIT				;AN000;
...
;-------------------------------------------------------------------------------;AN000;
    CALL  LOAD_PROFILE		    ;  Builds profile info in Temporary Shared	;AN000;
				    ;	Data					;AN000;
   jnc @F			    ; If error when loading the profile 	;AN000;
   				    ; then, EXIT				;AN000;
      JMP     ERROR_EXIT							;AN000;
   @@:										;AN000;
										;AN000;
...
;-------------------------------------------------------------------------------;AN000;
   cmp CS:[BP].PRINTER_TYPE,BLACK_WHITE						;AN000;
   jne @F
   test CS:[BP].SWITCHES,BACKGROUND_SW						;AN000;
   jz @F									;AN000;
...
;-------------------------------------------------------------------------------;AN000;
   cmp INSTALLED,NO		    ; If not installed				;AN000;
   jne @F			    ; then,					;AN000;
      MOV     END_OF_RESIDENT_CODE,OFFSET PRINT_MODULE_START			;AN000;
   @@:				    ;	we make everything up to the print	;AN000;
...
;-------------------------------------------------------------------------------;AN000;
   cmp INSTALLED,NO		    ; If not already installed			;AN000;
...
;------Replace the interrupt vectors						;AN000;
     ...
     cmp CS:[BP].HARDWARE_CONFIG,EGA;   If EGA is present			;AN000;
     jne @F			    ;	then,					;AN000;
...
ERROR_EXIT:									;AN000;
   cmp INSTALLED,YES		    ; If we are already installed, re-enable	;AN000;
   jne @F
...
cmp WORD PTR ES:[SI]+4,0	      ; IF the dynamic save are pointer is	;AN000;
jne endif_l_7
cmp WORD PTR ES:[SI]+6,0	      ;  null then, it's not defined            ;AN000;
jne endif_l_7			      ;   and we have to define it:		;AN000;
    ;---------------------------------------------------------------------------;AN000;
    ...
    cmp WORD PTR ES:[SI]+4,0FFH       ;If we can't read our byte back then,     ;AN000;
    je else_l_6			      ;  the Save Ptrs table is in ROM		;AN000;
       ;------------------------------------------------------------------------;AN000;
	...
	STI									;AN000;
    jmp endif_l_6
    else_l_6:			       ; ELSE save pointer table is in RAM	;AN000;
       ;------------------------------------------------------------------------;AN000;
    ...
    endif_l_6:			       ; ENDIF save pointer table is in ROM	;AN000;
  ;-----------------------------------------------------------------------------;AN000;
  ; Adjust END_OF_RESIDENT_CODE to the next offset available for copying	;AN000;
  ; resident code and data.							;AN000;
  ;-----------------------------------------------------------------------------;AN000;
...
;-------------------------------------------------------------------------------;AN000;
   ...
   MOV	   DI,OFFSET PRINT_MODULE_START ; ES:[DI] := Resident print modules	;AN000;
										;AN000;
  cmp INSTALLED,NO	 		; IF not installed			;AN000;
  jne else_l_10				; THEN, 				;AN000;
					;   We relocate the print modules	;AN000;
					;    at the end of the resident code:	;AN000;
					;     (this is where the color set is)	;AN000;
     cmp CS:[BP].PRINTER_TYPE,BLACK_WHITE ; IF we don't want the color set	;AN000;
     jne endif_l_10			;   THEN,				;AN000;
	...
	MOV CX,LEN_OF_BW_MODULES	;     CX      := Length of B&W modules	;AN000;
										;AN000;
  jmp endif_l_10
  else_l_10: 				; ELSE, (We are already installed)	;AN000;
     ...
     cmp AL,CS:[BP].PRINTER_TYPE	;   IF resident set is not the one	;AN000;
     je endif_l_9			;   we need THEN,			;AN000;
	MOV NEED_NEW_PRINT_MODULES,YES	;     Say we need a new set.		;AN000;
       cmp CS:[BP].PRINTER_TYPE,COLOR	;   IF its color we need then,	;AN000;
       jne else_l_8
	  ...
	  MOV CX,LEN_OF_BW_MODULES	;	CX	:= Length of B&W mod.	;AN000;
       endif_l_8:			;     ENDIF we need the color set	;AN000;
     endif_l_9:				;   ENDIF we need a new set		;AN000;
  endif_l_10:				; ENDIF we are not installed		;AN000;
										;AN000;
										;AN000;
;-------------------------------------------------------------------------------;AN000;
; If needed: Copy the required set of print modules				;AN000;
;-------------------------------------------------------------------------------;AN000;
  cmp NEED_NEW_PRINT_MODULES,YES						;AN000;
  jne @F 									;AN000;
     CLD			       ; Clear the direction flag		;AN000;
     REP     MOVSB		       ; Copy the set of print modules		;AN000;
  @@:				       ; ENDIF needs to copy the print modules	;AN000;
										;AN000;
;-------------------------------------------------------------------------------;AN000;
...
;-------------------------------------------------------------------------------;AN000;
  cmp INSTALLED,NO	 		; IF first time installed		;AN000;
  jne endif_l_12			; THEN, 				;AN000;
     MOV     CX,LEN_OF_COLOR_MODULES	;   Adjust END_OF_RESIDENT_CODE to	;AN000;
    cmp CX,LEN_OF_BW_MODULES		;   contains the larger set of modules. ;AN000;
    jle else_l_11			;					;AN000;
