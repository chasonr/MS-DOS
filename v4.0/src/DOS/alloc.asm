...
arena_free_next:
	CMP     BYTE PTR DS:[DI],arena_signature_end
					; end of road, Jack?
	jz      short ret_l_1           ; never come back no more
...
;
	...
	jz	short ret_l_2           ;   GOTO ok
	CALL    arena_next              ; ES, AX <- next block, Carry set if error
	jc	short ret_l_2           ; IF no error THEN GOTO check

coalesce_check:
	CMP     ES:[arena_owner],DI
	jnz	short ret_l_2           ; IF next block isnt free THEN return
