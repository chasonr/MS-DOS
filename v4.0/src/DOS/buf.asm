...
NORMSEC:
	InvokeFn DREAD			; Buffer is marked free if read barfs
...
; Carry set if problem (currently user FAILed to I 24)
	...
	CMP	[DI.buf_ID],AH
	jz	short ret_l_3		; Skip free buffers, carry clear
	CMP	AH,AL
	JZ	DOBUFFER		; Do all dirty buffers
	CMP	AL,[DI.buf_ID]
	CLC
	jnz	short ret_l_3		; Buffer not for this unit or SFT
DOBUFFER:
	TEST	[DI.buf_flags],buf_dirty
	jz	short ret_l_3		; Buffer not dirty, carry clear by TEST
	PUSH	AX
...
ASSUME	DS:NOTHING,ES:NOTHING
	...
	CMP	AL,0FFH
	jz	short ret_l_4		; Buffer is free, carry clear.
	TEST	AH,buf_dirty
	jz	short ret_l_4		; Buffer is clean, carry clear.
	CALL    DEC_DIRTY_COUNT 	; LB. decrement dirty count
	CMP	AL,BYTE PTR [WPERR]
	jz	short ret_l_4		; If in WP error zap buffer
...
skipwritetrans:
	...
	InvokeFn DWRITE		; Write out the dirty buffer
