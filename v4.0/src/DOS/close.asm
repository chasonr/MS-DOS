...
LocalClose:
	...
	InvokeFn FastSeek_Close	       ;FS. invoke fastseek			;AN000;
nofastsk:
	Context DS
	SaveReg <AX,BX>
	InvokeFn ShareEnd
...
do_update:				  ;AN005;
	Context DS
	InvokeFn FastOpen_Update 	; invoke fastopen
	POP	DX

;;; 10/1/86  update fastopen cache
	InvokeFn FLUSHBUF		; flush all relevant buffers
	RestoreReg  <DI,ES>
	MOV	AL,error_access_denied
	JC	CloseFinish
FREE_SFT_OK:
...
Procedure   DirFromSFT,NEAR
	...
	PUSH	DX
	InvokeFn FATREAD_SFT		; ES:BP points to DPB, [THISDRV] set
					; [THISDPB] set
	POP	DX
...
LOCAL_COMMIT:
	...
	call	CloseEntry
	PUSHF				;PTM.					;AN000;
	InvokeFn DEV_OPEN_SFT		;PTM.  increment device count		;AN000;
	POPF				;PTM.					;AN000;
...
;
	TEST	BX,devid_file_clean + devid_device
	jnz	short ret_l_2		; clean or device => no timestamp
;
; file and dirty.  See if date is good
;
	TEST	BX,sf_close_nodate
	jnz	short ret_l_2		; nodate => no timestamp
	SaveReg <AX,BX>
	InvokeFn DATE16			; Date/Time to AX/DX
