...
EndProc SPOOLINT
	...
	InvokeFn DSKSTATCHK		; Allows ^C to be detected under
					; input redirection
	PUSH	BX
	XOR	BX,BX
	InvokeFn GET_IO_SFT
...
ASSUME	SS:NOTHING
	MOV	SP,[user_SP]
	InvokeFn restore_world		; User registers now restored
ASSUME	DS:NOTHING
...
ctrlc_abort:
	MOV	AX,(EXIT SHL 8) + 0
	MOV	DidCTRLC,-1
	JMP	SHORT cmd_l_1		; give up by faking $EXIT
...
NoSetFail:
IF	BUFFERFLAG
	InvokeFn RESTORE_USER_MAP		;AN000;LB. restore user's EMS map
ENDIF
	MOV	[CONTSTK],SP
	Context ES
...
;;	InvokeFn SAVE_MAP		;LB. save user's EMS map                ;AN000;
...
;
	...
	InvokeFn Reset_Version		;AN007;MS. reset version number
...
;
	...
	InvokeFn $Get_interrupt_vector	; and who to go to
...
;
	...
	InvokeFn CAL_LK			; Set other extended error fields
	POP	SI
	ret
