...
;; FastSeek 10/27/86
	...
	InvokeFn RELEASE 		; Free file data
	JC	No_fileJ
; DOS 3.3  FastOpen
...
DELNXT:
	...
	InvokeFn GETENTRY		; Registers need to be reset
	JC	No_fileJ
	InvokeFn NEXTENT
if DEBUG
	JC	Flsh
...
ASSUME	ES:NOTHING
	...
	InvokeFn Set_Media_ID		;>32mb set voulme id to boot record	;AN000;
	InvokeFn FATRead_CDS		; force media check
	POP	DI
	POP	ES
	POP	AX
No_Set_Flag:
...
if installed
	...
	InvokeFn DOOPEN		; Fill in SFT for share check
	LES	DI,[THISSFT]
	...
	InvokeFn SHAREEND		; Tell sharer we're done with THISSFT
	CLC
CheckDone:
	LES	BP,[THISDPB]
	POP	SI
