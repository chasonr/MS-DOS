...
;-------------------------- Start of DBC;AN000;S 2/13/KK
	...
	InvokeFn TestKanj		;AN000;;IBMJ fix 9/04/86
...
;DOS 3.3 FastOPen 6/12/86 F.C.
	...
	MOV	BX,DX
	context DS
	InvokeFn SETDIRSRCH		; This uses UNPACK which might blow
	...
	XOR	AL,AL
	InvokeFn GETBUFFR		; Get the entry buffer back
SKIP_GETB:
	POP	CX
	POP	SI
	POP	AX
...
DoSplice:
	...
	InvokeFn FStrCpy 		;	strcpy (d, ThisCDS->Text);
DoChdir:
	Context DS
...
;
	...
	InvokeFn NameTrans		; advance DS:SI
...
;
	...
	JNZ	NotDevice
	LODSB
	InvokeFn PathChrCmp		; do we have the last path separator?
...
FASTINST:
	...
	InvokeFn Matchattributes 		    ;AN000;FO.;
