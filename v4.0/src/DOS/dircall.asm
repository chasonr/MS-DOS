...
ASSUME	DS:NOTHING
	...
	InvokeFn STARTSRCH	; Set search
	InvokeFn GETENTRY	; Get start of directory
...
ASSUME	DS:NOTHING
	...
	InvokeFn GETENTRY		; Get next entry
	JC	NOTDIRPATHPOP		; Screw up
	MOV	[ATTRIB],attr_directory+attr_hidden+attr_system
	InvokeFn SRCH			; Do a search
	...
	MOV	BX,[DIRSTART]
;; FastSeek 10/27/86
	InvokeFn Delete_FSeek		; delete the fastseek entry
;; FastSeek 10/27/86
	InvokeFn RELEASE 		; Release data in sub dir
	JC	NOTDIRPATHPOP		; Screw up
...
;DOS 3.3 FastOpen  6/16/86  F.C.
	PUSH	DS
	context DS
	InvokeFn FastOpen_Delete 	; call fastopen to delete an entry
	POP	DS
;DOS 3.3 FastOpen  6/16/86  F.C.
