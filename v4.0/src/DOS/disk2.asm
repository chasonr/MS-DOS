...
;	MOV	CL,ES:[BP.dpb_cluster_shift]
	...
	InvokeFn SHR32			     ; F.C. >32mb shift ax ;AN000;
...
SAVFIR:
	...
	OR	DX,[BYTCNT1]
	jnz	short ret_l_1	; NOT (BYTCNT1 = BYTCNT2 = 0)
	CMP	AX,1
	jnz	short ret_l_1
...
ERR_ON_CHECK:
	...
	JNE	NO_HARD_ERR
HARD_ERR:
	InvokeFn LOCK_VIOLATION
	jnc	short ret_l_2		; User wants Retrys
NO_HARD_ERR:
...
ENUF:
	InvokeFn CHECK_READ_LOCK 	;IFS. check read lock			 ;AN000;
...
ASSUME	DS:NOTHING
	PUSH	DX
	PUSH	CX
	InvokeFn SET_RQ_SC_PARMS 	 ;LB. do this for SC		       ;AN000;
...
IF	NOT	BUFFERFLAG
	...
	InvokeFn SET_MAP_PAGE		  ;LB. activate handle if EMS there	;AN000;
ELSE
;	int	3
	push	ds
...
NXTBUF: 			; Must see if one of these sectors is buffered
	InvokeFn BUFF_RANGE_CHECK			   ;F.C. >32mb
...
EVENMOV:
	...
	POP	DI
	MOV	AL,ES:[BP.dpb_drive]
	InvokeFn SCANPLACE		 ;LB. done with this chain		;AN000;
	JMP	SHORT end_scan		 ;LB.					;AN000;
CLBUFF:
	InvokeFn SCANPLACE
DONXTBUF:
...
SCAN_DONE:
	...
	POP	BX
	JCXZ	RDLAST
	InvokeFn IsEOF			; test for eof on fat size
	JAE	SETSFT
	MOV	DL,0
