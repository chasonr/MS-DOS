...
if debug
	...
	extern FCB_Ret_OK:NEAR,FCB_Ret_Err:NEAR
...
Procedure $Get_FCB_Position,NEAR
	...
	InvokeFn GetExtended		; point to FCB
	InvokeFn GetExtent		; DX:AX is current record
...
Procedure $FCB_Delete,NEAR
	...
	InvokeFn TransFCB		; convert FCB to path
	JC	BadPath 		; signal no deletions
	Context DS
	InvokeFn DOS_Delete		; wham
	JC	BadPath
GoodPath:
ok_ret_2:
...
Procedure $FCB_Close,NEAR
	...
	InvokeFn GetExtended		; DS:SI point to real FCB
...
Procedure $FCB_Rename,NEAR
	ASSUME	CS:DOSGROUP,DS:NOTHING,ES:NOTHING,SS:DOSGroup
	InvokeFn GetExtended		; get pointer to real FCB
	SaveReg <DX>
	...
	InvokeFn TransFCB		; munch this pathname
...
lru8:
	...
	InvokeFn CheckShare		; sharer here?
...
;
	InvokeFn CheckShare		; test for sharer
...
;
RegenNoSharing:
	InvokeFn CheckShare		; Sharing around?
	JNZ	RegenFail
;
...
DoneNam2:
	...
	MOV	[ATTRIB],attr_hidden + attr_system + attr_directory
					...
					; to call DEVNAME.
	InvokeFn DevName 		; check for device
	ASSUME	DS:NOTHING,ES:NOTHING
...
CheckD: AND	AL,3Fh
	...
	AND	AH,3Fh
	CMP	AH,AL
	jz	short ret_l_3		; carry is clear
BadSFT: STC
	return				; carry is clear
CheckNet:
;
...
Procedure   SFTFromFCB,NEAR
	...
	InvokeFn Save_World
	InvokeFn FCBRegen
	InvokeFn Restore_World		; restore world
	MOV	AX,EXTERR
	jnc	@F
	    ret
	@@:
