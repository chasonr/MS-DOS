...
FTRIM	EQU	2
	...
	InvokeFn GetExtended		;   FCB = GetExtended ();
	TEST	FCBOp,BLOCK		;   if ((OP&BLOCK) == 0)
	JNZ	GetPos
	MOV	CX,1			;	cRec = 1;
GetPos:
	MOV	cRec,CX 		;*Tail coalesce
	InvokeFn GetExtent		;   RecPos = GetExtent ();
	InvokeFn GetRecSize		;   RecSize = GetRecSize ();
	MOV	RecSize,BX
	TEST	FCBOp,RANDOM		;   if ((OP&RANDOM) <> 0)
	JZ	GetRec
	InvokeFn GetRR			;	RecPos = GetRR ();
GetRec:
	MOV	RecPosL,AX		;*Tail coalesce
	MOV	RecPosH,DX
	InvokeFn SetExtent		;   SetExtent (RecPos);
...
ENDIF
DoGetExt:
	InvokeFn SFTFromFCB		;	if (!SFTFromFCB (SFT,FCB))
	JNC	ContinueOp
FCBDeath:
	InvokeFn FCB_Ret_Err		; signal error, map for extended
...
;; Fix for disk full
	MOV	cResult,CX
	InvokeFn SaveFCBInfo		;	SaveFCBInfo (FCB);
...
SetExt:
	...
	JZ	TrySetRR
DoSetExt:
	InvokeFn SetExtent		;	SetExtent (RecPos, FCB);
TrySetRR:
	TEST	FCBOp,BLOCK		;   if ((op&BLOCK) <> 0)
	JZ	TryReturn
...
TryReturn:
	...
	InvokeFn Date16			;	FCB->FTime = time;
	RestoreReg  <DS>
	MOV	[SI].FCB_FDate,AX
	MOV	[SI].FCB_FTime,DX	;	}
FCBSave:
...
DoAccess:
	...
	InvokeFn TransFCB		; crunch the fcb
...
err_ret_1:
	JMP	FCB_Ret_Err
FindFCB:
	InvokeFn GetExtended		; DS:SI will point to FCB
	InvokeFn LRUFCB			; get a sft entry (no error)
	JC	HardMessage
	ASSUME	ES:NOTHING
...
DeadFCB:
	JMP	short err_ret_1
FCBOK:
	InvokeFn IsSFTNet		       ;AN007;F.C. >32mb  Non Fat file?
	JNZ	FCBOK2			       ;AN007;F.C. >32mb  yes
	InvokeFn CheckShare		       ;AN000;F.C. >32mb  share around?
...
OpenScan:
	...
	InvokeFn CheckFCB		; do they match
...
Procedure $FCB_Create,NEAR
	...
	InvokeFn GetExtended		; get extended FCB
