...
MACRO001E:
	...
	InvokeFn GetExtended		; get FCB, extended or not
...
NextCh: LOOP	FCBScan
 ENDIF
	RestoreReg  <SI>
	MOV	BX,DI
	InvokeFn PackName		; crunch the path
...
NO_CHECK:
	...
	InvokeFn PathChrCmp		; convert to normal form
...
;----------------------------- End of DBCS 2/23/KK
 ENDIF					;AN000;
	InvokeFn UCase			;AN000;; convert the char
	OR	AL,AL
	JZ	UNCTerm 		; end of string.  All done.
	InvokeFn PathChrCmp		; is it a path char?
...
UNCDone:
 ...
 IF  DBCS
;----------------------------- Start of DBCS 2/23/KK
	jc	short ret_l_2		;AN000; Return if error from Canonicalize
...
TP_check05:				;AN000;
	...
	InvokeFn testkanj		;AN000;; Kanji lead character?
...
TP_notK:				;AN000;
	InvokeFn PathChrCmp		;AN000;; Is it a path seperator char?
...
CheckPath:
	...
	InvokeFn CheckThisDevice 	; snoop for device
...
;
	...
	InvokeFn GetThisDrv		; set ThisCDS and init DUMMYCDS
	MOV	fSharing,0		;
;
...
;
	...
	MOV	AL,'/'                  ; path sep.
	STOSB
	InvokeFn StrCpy			; move remainder of string
	CLC				; everything OK.
...
DoFile:
	ASSUME	DS:NOTHING
	InvokeFn GetVisDrv		; get proper CDS
	MOV	AL,error_path_not_found ; Set up for possible bad file error
	jc	short ret_l_3		; CARRY set -> bogus drive/spliced
;
...
;
	...
	InvokeFn ValidateCDS		; poke CDS amd make everything OK
	RestoreReg <DI,ES,SI,DS>	; get back pointers
	MOV	AL,error_path_not_found ; Set up for possible bad path error
	jc	short ret_l_3		; someone failed an operation
;
...
;------------------------ End of DBCS 2/13/KK
 ELSE					;AN000;
	InvokeFn FStrCpy 		; copy string.	ES:DI point to end
...
DoCanon:
	...
	jc	short ret_l_3		; badly formatted path.
 IF  DBCS				;AN000;
;--------------------- Start of DBCS 2/13/KK
; Although Cononicalize has done lots of good things for us it may also have
; done e5 to 05 conversion on the fisrt char following a path sep char which is
...
; reconvert if found.
	...
	InvokeFn PathChrCmp		;AN000;; First char a path char?
	JNZ	TP_notremote		;AN000;; Not remote if not.
	InvokeFn PathChrCmp		;AN000;; Second char a path char?
	JNZ	TP_notremote		;AN000;; Not remote if not
...
;------------------------ End of DBCS 2/13/KK
 ENDIF					;AN000;
	...
	InvokeFn PathChrCmp		; did we end on a path char? (root)
...
;
	LODSB				; get the char
	InvokeFn PathChrCmp		; is it path char?
...
CanonPathLoop:
	LODSB				; get next byte
	InvokeFn PathChrCmp		; path char again?
...
Procedure   PathSep,NEAR
	...
	jz	short ret_l_4		; return if equal to zero (NUL)
	InvokeFn PathChrCmp		; check for path character
...
NormalComp:				;   else {
	MOV	SI,CopySoff
	InvokeFn NameTrans		;	s = NameTrans (s, Name1);
...
DoPack: 				;	    }
	...
	LEA	DI,CopyTemp
	SaveReg <DI>
	InvokeFn PackName		;	PackName (Name1, temp);
	RestoreReg  <DI>
	InvokeFn StrLen			;	if (strlen(temp)+d > bp)
	DEC	CX
	ADD	CX,CopyDoff
	CMP	CX,CopyBP
...
;----------------------- End of DBCS 2/13/KK
 ELSE					;AN000;
	REPZ	CMPSB			; compare
	jnz	short ret_l_6		; if NZ then return NZ
	SaveReg <AX>			; save char register
 ENDIF					;AN000;
	MOV	AL,[SI-1]		; get last byte to match
	InvokeFn PathChrCmp		; is it a path char (Root!)
...
Procedure     ScanPathChar,NEAR
 ...
 IF  DBCS				;AN000;
	InvokeFn TestKanj		;AN000;; 2/13/KK
...
NotKanjr:				;AN000;; 2/13/KK
 ENDIF					;AN000;
	...
	InvokeFn PathChrCmp		; path separator?
	return
