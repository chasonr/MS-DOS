...
;Same as above except drive passed in DL (0=default, 1=A, 2=B, ...)
	...
	context DS
	MOV	AL,DL
	InvokeFn GETTHISDRV		; Get CDS structure
SET_AL_RET:
...
; Same as above only drive passed in DL (0=default, 1=A, 2=B, ...)
	...
	EnterCrit CritDisk
	InvokeFn FATRead_CDS		; Force Media Check and return DPB
	LeaveCrit CritDisk
...
Create_dup_jfn:
	...
	InvokeFn SFFromHandle		; get sf pointer
...
;
	...
	InvokeFn DOS_DUP 		; signal duplication
;
; get the old sfn for copy
;
	InvokeFn pJFNFromHandle		; ES:DI is jfn
...
doioctl:			      ;AN000;
	...
	InvokeFn $IOCTL		      ;AN000;MS.; let IOCTL take care of it
	return			      ;AN000;MS.;
errorfunc:			      ;AN000;
