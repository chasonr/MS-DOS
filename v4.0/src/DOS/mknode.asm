...
EVENZ:
        ...
        InvokeFn INC_DIRTY_COUNT          ;LB.                                  ;AN000;
...
make_new:
        call    make_save
        jc	short ret_l_3   ; case 2 fail
        TEST    BYTE PTR [ATTRIB],attr_directory
        jnz	short ret_l_3   ; Don't "open" directories, so don't
...
yesdirty2:
        ...
        InvokeFn FlushBuf               ; write out buffer.
        POP     AX
        jmp     make_Share_ret
;
...
MakeEndShare:
        ...
        SaveReg <AX,DI,ES>
        PUSHF
        InvokeFn ShareEnd               ; remove sharing
        POPF
...
ASSUME  ES:NOTHING
        ...
        CMP     [FAILERR],0
        STC
        jnz	short ret_l_4   ; User FAILed, node might exist
        CALL    BUILDDIR        ; Try to build dir
        jc	short ret_l_4   ; Failed
        InvokeFn GETENT         ; Point at that free entry
        jc	short ret_l_4   ; Failed
        JMP     SHORT FREESPOT
...
;       InvokeFn Get_Global_CdPg ;FT. get global code page                       ;AN000;
...
; Do nothing for a device.
        ...
        InvokeFn Dev_Open_SFT   ; increment ref. count
...
GotADevice:
        ...
        InvokeFn SET_MAP_PAGE   ;LB. remap curbuf                               ;AN000;
...
ASSUME  DS:NOTHING
        ...
        MOV     BX,CX
        InvokeFn Delete_FSeek   ; FS. delete Fastseek Clusters                  ;AN000;
        InvokeFn RELEASE        ; Free any data allocated
        POP     DX
...
GET_BUF_BACK:
        ...
        XOR     AL,AL
        InvokeFn GETBUFFR       ; Get sector back
        POP     BX              ; Get offset back
        jc	short ret_l_6
        InvokeFn SET_BUF_AS_DIR
