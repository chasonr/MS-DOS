...
ret_l_1:
	...
	InvokeFn SET_RQ_SC_PARMS 	;LB. set up SC parms			;AN000;
	PUSH	DS
	PUSH	SI
	PUSH	AX
	Context DS
...
ASSUME	SS:DOSGROUP
	...
	InvokeFn Save_World		      ;>32mb save all regs		;AN000;
	PUSH	ES
	CALL	AbsSetup
	JC	ILEAVE
if not ibmcopyright
...
ILEAVE:
	POP	ES
	InvokeFn Restore_World		     ;>32mb				;AN000;
	CLI
	DEC	INDOS
	MOV	SS,[user_SS]
ASSUME	SS:NOTHING
...
entry	SYS_RET_OK
	InvokeFn FETCHI_CHECK		; TAG checking for FETCHI
...
dofast:
	...
	InvokeFn Fast_Dispatch		      ; call fastopen/seek		;AN000;
nofast:
	POP	DX								;AN000;
