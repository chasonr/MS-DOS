...
DoCheck:
	...
	InvokeFn ValidateCDS		; output is ES:DI -> CDS
...
;	AX = error_current_directory
	...
	push	di
	InvokeFn TransPathNoSet		; Translate the name
...
rmloop: InvokeFn GetCDSFromDrv		; Get curdir for drive in al
	jc	rmcont			; If error, exit loop & cont normally
	InvokeFn StrCmp			; Are the 2 paths the same?
...
;	AX = chdir_path_not_found if error
	...
	InvokeFn TransPath		; go munge the path and get real CDS
...
;
	...
	InvokeFn Get_User_Stack		; get original text
	ASSUME	DS:NOTHING
	MOV	DI,[SI.User_DX]
	MOV	DS,[SI.User_DS]
	...
	SaveReg <DI>
	InvokeFn TransPathNoSet		; Munge path
	RestoreReg  <SI>
	Assume	DS:DOSGroup
;
...
DoDirCall:
	...
	SaveReg <SI>
	MOV	SI,DX			; get source
	InvokeFn TransPath		; go munge the path
	RestoreReg  <SI>
