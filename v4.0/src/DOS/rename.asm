...
; DS preserved, others destroyed
	...
	InvokeFn TestNet
	JNC	LOCAL_RENAME
;	InvokeFn OWN_SHARE2		       ;IFS.  IFS owns share ?		;AN000;
;	JZ	ifsshare		       ;IFS.  yes			;AN000;
;	PUSH	WORD PTR [DMAADD+2]	       ;IFS.  save DMAADD		;AN000;
;	PUSH	WORD PTR [DMAADD]	       ;IFS.				;AN000;
;
;	InvokeFn IFS_SEARCH_FIRST	       ;IFS.  search source name	;AN000;
;	JC	nofiles 		       ;IFS.  not found 		;AN000;
rename_next_file:
;	InvokeFn IFS_REN_DEL_CHECK	       ;IFS.  do share check		;AN000;
...
;	InvokeFn DOS_SEARCH_NEXT 	       ;IFS.  serch next source 	;AN000;
...
SAMEDRV:
	...
	EnterCrit   critDisk
	InvokeFn DOS_SEARCH_FIRST	; Sets [NoSetDir] to 1, [CURBUF+2]:BX
					;    points to entry
	JNC	Check_Dev
...
Check_dev:
	...
	InvokeFn Check_Pathlen2		      ;PTM.   length < 67		;AN000;
notdir:
	POP	DS			      ;PTM.				;AN000;
	JA	GOTERR			      ;PTM.				;AN000;
...
; [SATTRIB] is also set.
	MOV	SI,BX
	ADD	SI,dir_first
	InvokeFn REN_DEL_Check
	JNC	REN_OK1
	MOV	AX,error_sharing_violation
	JMP	RENAME_POP

REN_OK1:				;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	InvokeFn FastOpen_Delete 	; delete dir info in fastopen DOS 3.3
...
BUILDDEST:
	...
	MOV	[CREATING],0FFH
	InvokeFn DEVNAME 		; Check if we built a device name
	ASSUME	ES:NOTHING
	JNC	BAD_ACC
	MOV	BX,[DESTSTART]
	LES	BP,[THISDPB]
	InvokeFn SetDirSrch		; Reset search to start of dir
	JC	BAD_ACC 		; Screw up
	InvokeFn FINDENTRY		; See if new name already exists
	...
	InvokeFn RENAME_MAKE		; This is in mknode
	JNC	GOT_DEST
BAD_ACCJ:
	JMP	BAD_ACC
...
GOT_DEST:
	...
	InvokeFn ShareEnd		; we need to remove it.
	RestoreReg  <BX>
