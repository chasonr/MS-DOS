...
entry   SKPCLP
        ...
        MOV     [Temp_Var2],BX                    ; save physical cluster       ;AN000;
                                                  ; PTR P005079
SKPCLP2:
        InvokeFn fastseek_lookup                  ; ask for next cluster #      ;an000;
...
noteof:
        ...
        InvokeFn fastseek_insert                  ; insert cluster # to         ;an000;
...
EVENRD:
        ...
        InvokeFn PLACEHEAD              ; Make it prime candidate for chucking
...
;; 11/5/86 FastSeek
        ...
        JZ      do_norm2                ; no                                    ;AN000;
look2:                                                                          ;AN000;
        InvokeFn FastSeek_Lookup        ; call lookup                           ;AN000;
...
do_norm2:
        InvokeFn UNPACK
        JC      NONEXT
        InvokeFn FastSeek_Insert        ; call insert                           ;AN000;
...
;       USES    ALL but SI, BP
        ...
        POP     BX
        jc	short ret_l_2           ; abort if error   [INTERR?]
...
aupk:
        InvokeFn UNPACK         ; check out this cluster
...
;       (DX) = current last cluster in file
        ...
        InvokeFn PACK           ; set special "temporary" mark
        jc      ads4            ; FAT error             [INTERR?]
...
NO_ALLOC:
        ...
        XCHG    BX,DX
        MOV     AX,DX
        InvokeFn PACK           ; link free cluster onto file
...
;       (BX) = last cluster in file
        ...
        MOV     DX,0FFFFH
        InvokeFn PACK           ; mark last cluster EOF
...
ads4:   POP     BX
        ...
        jc	short ret_l_2
        InvokeFn UNPACK         ; Get first cluster allocated for return
        ...
        OR      DI,DI           ; clear 'C'
        jnz	short ret_l_2   ; we were extending an existing file
...
dofastk:
        ...
        InvokeFn FastSeek_Open                    ; create this file entry
        POP     CX
do_norm5:
        POP     ES
;; 11/6/86 FastSeek
...
NO_DEALLOC:
        MOV     BX,AX
        dec     ax              ; check for "1"
        jz	short ret_l_3   ; is last cluster of incomplete chain
        InvokeFn IsEOF
...
;       carry clear: EOF and do truncate
        ...
        InvokeFn iseof                            ; is eof                      ;an000;
        JB      noteof2                           ; no                          ;AN000;
        PUSH    [FSeek_logclus]                   ;                             ;AN000;
        PUSH    [FSeek_logsave]                   ; logclus=logsave             ;AN000;
        POP     [FSeek_logclus]                   ; delete EOF                  ;AN000;
        InvokeFn fastseek_truncate                ;                             ;an000;
