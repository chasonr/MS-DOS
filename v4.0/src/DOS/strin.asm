...
;
        OR      AL,AL
        jz	short ret_l_1   ;Buffer is 0 length!!?
...
NEWLIN:
        ...
        InvokeFn $STD_CON_INPUT_NO_ECHO         ;Get first char
...
;
entry   GETCH
        InvokeFn $STD_CON_INPUT_NO_ECHO
GOTCH:
;
...
SAVCH:
        ...
        InvokeFn BUFOUT                 ;Print control chars nicely
...
OLDBAK:
        CMP     BYTE PTR [INSMODE],0
        jnz	short ret_l_11  ;In insert mode, done
        OR      BH,BH
        jz	short ret_l_11  ;Not advanced in template, stay where we are
...
FINDOLD:
        ...
        InvokeFn $STD_CON_INPUT_NO_ECHO ; eat next char
...
entry   REEDIT
        ...
        PUSH    ES
        PUSH    DS
        InvokeFn COPYNEW        ;Copy current line into template
        POP     DS
        POP     ES
        POP     SI
