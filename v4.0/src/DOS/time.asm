...
;
	...
	InvokeFn get_user_stack		;Get pointer to user registers
ASSUME	DS:NOTHING
...
;	AL = -1 date bad, = 0 OK
	...
	jc	short ret_l_1		;Error if not big enough
	CMP	CX,119			;Year must be less than 2100
	JA	RET24
	OR	DH,DH
	jz	short ret_l_1
	OR	DL,DL
	jz	short ret_l_1		;Error if either month or day is 0
...
;	AL = -1 time bad, = 0 OK
	...
	LDS	SI,[BCLOCK]
ASSUME	DS:NOTHING
	InvokeFn DEVIOCALL2		;Get correct day count
	POP	DS
	...
	LDS	SI,[BCLOCK]
ASSUME	DS:NOTHING
	InvokeFn DEVIOCALL2		;Set the time
	XOR	AL,AL
	return
EndProc $SET_TIME
...
;Uses AX, CX, DX.
	...
	LDS	SI,[BCLOCK]
ASSUME	DS:NOTHING
	InvokeFn DEVIOCALL2		;Get correct date and time
	POP	DS
...
EndProc SETYEAR
	...
	jc	short ret_l_4		;Error if too many days
	CALL	SETYEAR
;
; WARNING!  DAY and MONTH must be adjacently allocated
;
...
ASSUME	DS:NOTHING
	...
	LDS	SI,[BCLOCK]
ASSUME	DS:NOTHING
	InvokeFn DEVIOCALL2		;Set the date
	POP	DS
