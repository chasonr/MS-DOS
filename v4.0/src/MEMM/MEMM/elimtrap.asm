...
PgN_FF:
	xor	bh, bh				; make BX = flip-flop state
        ...
        mov     byte ptr DMARegSav.DMALinAdr.HighWord_[si], al ; save value
        call    DMA_DMAFixup                    ; Translate Lin to Phys
						; & Update DMARegSav
	call	DMA_WrtAdrReg 		        ; emulate the write
	jmp	short DPExit
Pg_rd_port:
        mov     al, byte ptr DMARegSav.DMALinAdr.HighWord_[si]
        mov     [SaveAL], al
DPExit:
        pop     dx
        pop     cx
...
SaveIt:
	...
	xor	dh, dh
        ...
        mov     al, byte ptr DMARegSav.DMAPhyAdr.HighWord_[si]
        out     dx, al
...
; Must Update Page and Base registers simultaneously when remapping occurs...
	...
	xor	dh, dh				; clear high byte
        ...
        xor     bx, 1                           ; toggle flip-flop
						...
						; Page Register...
        mov     al, byte ptr DMARegSav.DMAPhyAdr.HighWord_[si] ; fetch value
        mov     dl, byte ptr DMARegSav.DMAPagePort[si]
        out     dx,al                           ; output the byte
