...
;;  nop
    ...
    ;**********************************************************************
    cmp SEGMENT_SIZE,MINIMUM_MEMORY		 ;AN000; Is there enough memory?
    jae @F
    ...
    DOSCALL					 ;AN000;
    jnc @F					 ;AN000; Was there an error opening the file?
    ...
    jnc @F					 ;AN000; Was there an error reading from the file?
...
PARSE_NEXT_LINE:				 ;AN000;
    ...
    @@:						 ;AN000;
    test PARSE_FLAG,FIRST_PARSE			 ;AN000; Looking for a value or a string?
    jz endif_l_1
	 cmp AX,0				 ;AN000; Was there an error?
	 je @F
    ...
    ;**********************************************************************
    cmp AX,0					 ;AN000; Was an error found parsing the buffer
    jne endif_l_2
	 cmp N_PRN_NAMES,MAX_NUM_PRINTER_DEFS	 ;AN000; Are there more the 255 printer defs?
	 jb @F
    ...
    cmp CURRENT_PARSE_LOC,0			 ;AN000; Is there more data?
    jne @F
...
EXIT_WITHOUT_ERROR:				 ;AN000;
    ...
    cmp CL,NUM_PRINTER_DEFS			 ;AN000; Did the number of definitions agree with the expected number?
    je @F
...
SEARCH_LINE   PROC NEAR 	    ;AN000;
    ...
    jnc endif_l_4				 ;AN000; Was the END OF LINE found?
	 test READ_FLAG,AT_EOF		 	 ;AN000; Are we at the end of the file?
	 jz else_l_3
	      MOV  START_NEXT_LINE, 0		 ;AN000; Yes!  Indicate that this is the last line
	 jmp short endif_l_3
	 else_l_3:				 ;AN000; We are not the the end of the file
...
SCAN_FOR_EOLN PROC NEAR;AN000;
    ...
    while_l_1:
    cmp CX,0					 ;AN000; Search until the end of the data
    jbe endwhile_l_1
	 cmp BYTE PTR ES:[DI],E_FILE_TERM	 ;AN000;
	 jne endif_l_6
	      or AL,AL		;AN000;
	      jnz @F
	      ...
	      JMP  EXIT_SCAN   ;AN000;
	 endif_l_6: 	       ;AN000;
	 cmp BYTE PTR ES:[DI],E_LF		 ;AN000; Is this character a CR or a LF?
	 je @F
	 cmp BYTE PTR ES:[DI],E_CR		 ;AN000;
	 jne elseif_l_7
	 @@:
	      or AL,AL				 ;AN000; Has a CR or a LF already been found?
	      jnz @F
		   MOV	END_CUR_LINE, DI	 ;AN000; Save this location in the string
		   DEC	END_CUR_LINE		 ;AN000; Point to the real end of the line
	      @@:	       ;AN000;
	 ...
	 jmp short endif_l_7
	 elseif_l_7:
	 or AL,AL		 		 ;AN000; If we have passed the CR-LF,
	 jz endif_l_7
    ...
    cmp AL,2					 ;AN000; Does the data run out right after the CR-LF?
    jb @F
...
COPY_PRINTER_NAME  PROC NEAR;AN000;
    ...
    cmp AL,1					 ;AN000; Was the parameter a P?
    jne else_l_8
	 MOV  AL, 'P'                            ;AN000; Yes!
    jmp short endif_l_8
    else_l_8:					 ;AN000;
    ...
    INC  CX					 ;AN000; Get the length of the line
    cmp CX,E_MAX_PRN_NAME_LEN			 ;AN000; Is the length of the line too long?
    jbe @F
    ...
    while_l_2:
    or CX,CX					 ;AN000; Do while there are more characters in the string
    jz endwhile_l_2
	 MOV  AL, BYTE PTR ES:[SI]		 ;AN000; Get a character from the file data
	 cmp AL,32				 ;AN000; Is this character a space?
	 je endif_l_9
	      MOV  BX, DX			 ;AN000; Get the line length
	      SUB  BX, CX			 ;AN000; Get the length of the line copied so far
	      INC  BX				 ;AN000;
	      cmp MAX_NAME,BX		     ;AN000; Is this name longer than the longest so far?
	      jae @F
...
READ_FROM_HERE	   PROC NEAR;AN000;
    ...
    jnc @F					 ;AN000; Was an error encountered?
    ...
    cmp AX,BUFFER_SIZE				 ;AN000; Were less bytes read then we asked for?
    jae @F
...
GET_PRINTER_INFO_ROUTINE     PROC FAR;AN000;
    ...
    ;**********************************************************************
    cmp DX,WORD PTR FILE_PTR_AT_START+2		 ;AN000; Compare the high order words
    jae else_l_10
	 JMP  READ_MORE_INFORMATION		 ;AN000; Info in buffer is passed where we want.
    jmp short endif_l_10
    else_l_10:					 ;AN000;
	 cmp DX,WORD PTR FILE_PTR_AT_START+2	 ;AN000; High words equal so
	 jne @F
	 cmp CX,WORD PTR FILE_PTR_AT_START	 ;AN000; compare the low order words
	 jae @F
	      JMP  READ_MORE_INFORMATION	 ;AN000; Info in buffer is passed where we want.
	 @@:	 			  ;AN000;
    endif_l_10:			   ;AN000;
    ...
    cmp DX,BX					 ;AN000; If data we want is further in the file,
    jbe else_l_11
	 JMP  READ_MORE_INFORMATION		 ;AN000; Read in more data
    jmp short endif_l_11
    else_l_11:		       ;AN000;
	 cmp DX,BX				 ;AN000; If the high words are equal do the
	 jne @F
	 cmp CX,SI				 ;AN000; comparison on the low order words
	 jbe @F
    ...
    cmp AL,NUM_PRINTER_DEFS			 ;AN000; Is this the last one in the list?
    jne endif_l_13
	 test READ_FLAG,AT_EOF			 ;AN000; If it is, and all the data has been read, process it
	 jz else_l_12
    ...
    cmp DX,BX					 ;AN000; See if the next printer name is in the buffer
    jbe else_l_14
	 JMP  READ_MORE_INFORMATION		 ;AN000; If not, get more information
    jmp short endif_l_14
    else_l_14:		       ;AN000;
	 cmp DX,BX				 ;AN000; If the high order words are equal,
	 jne @F
	 cmp CX,SI				 ;AN000; Compare the low order.  Is the name there?
	 jbe @F
...
READ_MORE_INFORMATION:				 ;AN000;
    ...
    jnc @F					 ;AN000; If CY = 1, there was an error
    ...
    @@:		 ;AN000;
    cmp AX,BUFFER_SIZE				 ;AN000; Were less bytes read then asked for?
    je else_l_15
	 OR   READ_FLAG, AT_EOF 		 ;AN000; If so, then we are at the end of file
    jmp short endif_l_15
    else_l_15:					 ;AN000; Otherwise...
...
PARSE_NEXT_PARM:		  ;AN000;
    ...
    POP  ES					 ;AN000; Restore the file data segment
    POP  DS					 ;AN000; Restore the data segment
    cmp AX,0FFFFH				 ;AN000; Was the end of the line found?
    jne elseif_l_16
	 JMP  UPDATE_PARSE_PTR			 ;AN000; If so, start the next one
    jmp short endif_l_16
    elseif_l_16:
    or AX,AX					 ;AN000; Was an error encountered parsing the line?
    jz endif_l_16
    ...
    cmp AL,1					 ;AN000; Was this a printer name and type?
    je @F
    cmp AL,2		   ;AN000;
    jne elseif_l_18_1
    @@:
	 test PARSE_FLAG,FIRST_NAME		 ;AN000; Is this the first name encountered?
	 jz else_l_17
	      AND  PARSE_FLAG, RESET_FIRST_NAME  ;AN000; Indicate that the a name has been found
	 jmp short endif_l_18
	 else_l_17:				 ;AN000; Otherwise...
	      JMP  PARSING_DONE 		 ;AN000; This is the second name.  We are finished.
	 endif_l_17: 		   ;AN000;
    jmp short endif_l_18
    elseif_l_18_1:
    cmp AL,3		 			 ;AN000; AL = 3 ==> Mode parameters
    jne elseif_l_18_2
	 CALL HANDLE_MODE			 ;AN000; Process
    jmp short endif_l_18
    elseif_l_18_2:
    cmp AL,4		 			 ;AN000; AL = 4 ==> Code page driver parameters
    jne elseif_l_18_3
	 MOV  CDP_FOUND, TRUE	   ;AN000;
	 CALL HANDLE_CODE_DRIVER		 ;AN000; Process
    jmp short endif_l_18
    elseif_l_18_3:
    cmp AL,5		 			 ;AN000; AL = 5 ==> Code page preparation parameters
    jne elseif_l_18_4
	 MOV  CPP_FOUND, TRUE	   ;AN000;
	 CALL HANDLE_CODE_PREPARE		 ;AN000; Process
    jmp short endif_l_18
    elseif_l_18_4:
    cmp AL,6		 			 ;AN000; AL = 6 ==> Graphics parameters
    jne endif_l_18
...
UPDATE_PARSE_PTR:	  ;AN000;
    ...
    cmp CURRENT_PARSE_LOC,0			 ;AN000; Is there any more data?
    jne @F
...
COPY_RESULT   PROC NEAR;AN000;
    ...
    while_l_3:
    cmp CX,AX					 ;AN000; Copy up to the number of character in AX
    jae endwhile_l_3
	 MOV  DL, BYTE PTR ES:[SI]		 ;AN000; Get the first byte
	 or DL, DL				 ;AN000; Terminate loop if this is a null char
	 jz endwhile_l_3
...
COPY_LINE     PROC NEAR;AN000;
    ...
    cmp AX,CX					 ;AN000; If length is greater then the maximum allowed,
    jae @F
...
PARSE_FOR_SELECT:				 ;AN000; Here to parse the line
    ...
    jnc @F					 ;AN000; Was there an error?
	 JMP  AUTO_EXIT_ERROR			 ;AN000; Yes! Exit the subroutine
    @@:						 ;AN000;
    cmp AX,0					 ;AN000; Did the parser file 'SELECT'?
    jne else_l_19
	 ...
	 MOV  AX, 4200H 	 ;AN000;
	 DOSCALL		 ;AN000;
	 jnc @F					 ;AN000; Was there an error writing the data?
	      JMP  AUTO_EXIT_ERROR		 ;AN000; Yes! Exit the subroutine
	 @@:	 				 ;AN000;
	 ;*****************************************************************
    ...
    else_l_19:			     ;AN000;
	 ...
	 cmp CURRENT_PARSE_LOC,0	 	 ;AN000; If this is zero, then there is no more data
	 jne @F
