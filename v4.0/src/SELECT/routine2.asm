...
CHECK_DOS_PATH_ROUTINE	PROC FAR		 ;AN000;
    ...
    cmp AX,0					 ;AN000; If the length is zero then return that
    jne @F
    ...
    ADD  SI, 2					 ;AN000; Adjust path pointer for length word
						 ;
    repeat_l_1:					 ;AN000; Check all the path names in the string
...
CHECK_VALID_PATH   PROC FAR			 ;AN000;
    ...
    jnc @F					 ;AN000; Is the drive specified invalid?
	 MOV  AX, ERR_INVALID_DRV		 ;AN000; Return this error code
	 JMP  EXIT_CHK_DRV			 ;AN000; Exit the subroutine
    @@:						 ;AN000;
    cmp BX,0					 ;AN000; No drive sepecified?
    jne else_l_1
	 cmp AL,2				 ;AN000; Must the drive be specified?
	 jne endif_l_1
	      MOV  AX, ERR_NO_DRIVE		 ;AN000; Return this error code
	      JMP  EXIT_CHK_DRV 		 ;AN000; Exit the subroutine
    jmp short endif_l_1				 ;AN000;
    else_l_1:					 ;AN000; Otherwise, the drive WAS specified.
	 cmp AL,0				 ;AN000; The drive cannot be specified
	 jne @F
    ...
    endif_l_1:					 ;AN000;
    cmp BYTE PTR [SI],'\'			 ;AN000; Is the next byte a backslash?
    jne else_l_4
	 cmp AH,0				 ;AN000; Is one permitted?
	 jne else_l_2
	      MOV  AX, ERR_LEADING_SLASH	 ;AN000; No! Return this error code
	      JMP  EXIT_CHK_DRV 		 ;AN000; Exit the subroutine
	 jmp short endif_l_4
	 else_l_2:				 ;AN000; Otherwise, one allowed.
	      INC  SI				 ;AN000; Push pointer past \
	 endif_l_2: 				 ;AN000;
    jmp short endif_l_4
    else_l_4:					 ;AN000; Otherwise, byte not a backslash
	 cmp AH,2				 ;AN000; Was one required?
	 jne endif_l_3
    ...
    repeat_l_2:					 ;AN000; Check all the path names in the string
	 MOV  AL, '\'                            ;AN000; Separator between filenames
	 MOV  CX, PATH_STRING			 ;AN000;
	 ADD  CX, STRING_SIZE			 ;AN000;
	 SUB  CX, SI				 ;AN000;
	 cmp NUM_PATHS,0			 ;AN000; If this is the first path checked...and
	 jne @F
	 cmp CX,0				 ;AN000; If the length of the path is zero...
	 jne @F
	 ...
	 CALL RESTORE_SEPARATOR 		 ;AN000; Restore the character between the path names
	 jnc endif_l_6				 ;AN000; Was the file name not valid?
	      cmp NUM_PATHS,0			 ;AN000;
	      jne else_l_5
		   jmp short endrepeat_l_2	 ;AN000;
	      jmp short endif_l_5
	      else_l_5:				 ;AN000;
    ...
    cmp  BYTE PTR [SI],'\'                       ;AN000; Is the last character a \ ?
    jne @F
...
CHECK_VALID_DRIVE  PROC NEAR			 ;AN000;
    ...
    cmp BYTE PTR [SI+1],':'			 ;AN000; Is the second character in the string a ':'
    jne else_l_8
	 MOV  AL, [SI]				 ;AN000; If so, get the first character
	 cmp AL,'A'				 ;AN000; Is it a capital letter?
	 jb elseif_l_7
	 cmp AL,'Z'				 ;AN000;
	 ja elseif_l_7
	 ...
	 jmp short endif_l_8
	 elseif_l_7:
	 cmp AL,'a'				 ;AN000; Else, is the drive a lowercase letter?
	 jb else_l_7
	 cmp AL,'z'				 ;AN000;
	 ja else_l_7
...
CHECK_VALID_FILENAME	PROC NEAR		 ;AN000;
    ...
    jmp while_l_1				 ;AN000;
    endwhile_l_1:
    cmp CHAR_COUNT,0				 ;AN000;
    jne @F
    cmp MAX_CHAR,8				 ;AN000;
    jne @F
	 DEC  NUM_PATHS 			 ;AN000;
	 JMP  INVALID_CHAR			 ;AN000;
    @@:						 ;AN000;
...
CHANGE_ATTRIBUTE_ROUTINE     PROC FAR		 ;AN000;
    ...
    cmp DI,BX					 ;AN000;
    jae endwhile_l_2
	 cmp WAY,0				 ;AN000; Setting the attribute?
	 jne else_l_13
	 ...
	 else_l_13:				 ;AN000; Otherwise, we are restoring the attribute
...
COMPARE_ROUTINE    PROC FAR			 ;AN000;
    ...
    cmp [DI],CX					 ;AN000; Are the lengths of the strings the same?
    je @F
...
CHECK_WRITE_ROUTINE	PROC FAR		 ;AN000;
    ...
    cmp DRIVE_FLAG,DRIVE_A			 ;AN000;JW
    jne else_l_14
    ...
    jnc else_l_16				 ;AN000; Was there an error?
	 cmp W_PROTECT_FLAG,TRUE	 	 ;AN000; If the INT 24H handler was call...
	 jne else_l_15
	...
	jne @F
	     JMP  WRITE_PROTECTED		 ;AN000; The disk is write protected.
	@@:					 ;AN000;
    endif_l_16:					 ;AN000; There were no errors...
...
MATCH_FILES_ROUTINE	PROC FAR		 ;AN000;
    ...
    cmp LIST_TYPE,2				 ;AN000; If this list is a 14 byte list...
    jne @F
    ...
    MOV  BX, 0					 ;AN000; Initialize the count of files checked
						 ;
    while_l_3:
    cmp BX,NUM_FILES				 ;AN000; Perform NUM_FILES interations
    jae endwhile_l_3
	 CLD					 ;AN000;
...
CHECK_DISK_ROUTINE PROC FAR			 ;AN000;
    ...
    test BX,M_EDOS_EXISTS			 ;AN000; Does the extended DOS partition exist?
    jz elseif_l_24
	 test BX,M_EDOS_SPACE			 ;AN000; Yes! Is there free space in it?
	 jz elseif_l_23
	      MOV  DX, FREE_EDOS_SPACE		 ;AN000; Indicate that there is free space
	 jmp short endif_l_24
	 elseif_l_23:
	 test BX,M_FREE_SPACE			 ;AN000; Is there no free space on the disk?
	 jnz endif_l_24
	      MOV  DX, NO_EDOS_SPACE		 ;AN000; Indicate there is no free space in EDOS or on the disk.
	 endif_l_23: 				 ;AN000;
    jmp short endif_l_24
    elseif_l_24:
    test BX,M_FREE_SPACE			 ;AN000; No! There is no EDOS partition
    jz endif_l_24
...
CHECK_VALID_MEDIA_ROUTINE    PROC FAR		 ;AN111;JW
    ...
    cmp VAR_DISK_A,E_DISKETTE_INV		 ;AN111; Is disk A present
    je else_l_28
	 cmp VAR_DISK_B,E_DISKETTE_INV		 ;AN111; Is disk B present
	 je else_l_27
	      cmp VAR_DISK,0			 ;AN111; Hard disk is present?
	      jle else_l_25
