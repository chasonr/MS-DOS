...
PUSH_ROUTINE	PROC	FAR			;AN000;
	...
	cmp STACK_INDEX,STACK_SIZE		;AN000; Is there any room?
	jae @F
...
POP_ROUTINE    PROC    FAR			;AN000;
	...
	cmp STACK_INDEX,0	 		;AN000; Is there anything on the stack?
	jbe else_l_2
...
PREPARE_FILE_ROUTINE	PROC FAR		;AN000;
    ...
    ;**********************************************************************
    jnc endif_l_4				;AN000; Did the DOS fn. return an error?
	 cmp AX,5				;AN000; Was the error ACCESS DENIED?
	 jne else_l_3
	      MOV  DX, BX			;AN000; Get the address of the string
	      MOV  INT_24_ERROR, 0		;AN000; Zero the number of critical errors
	      MOV  AX, 3D01H			;AN000; DOS Fn. for opening a file
	      DOSCALL				;AN000;
	      jnc endif_l_3 			;AN000; Was there an error?
...
WRITE_LINE_ROUTINE PROC FAR			;AN000;
    ...
    cmp N_WRITE_ERR_CODE,0			;AN000; Has there been an error?
    jne endif_l_6
...
SAVE_FILE_ROUTINE  PROC FAR			;AN000;
						;
    cmp N_WRITE_ERR_CODE,0			;AN000; Has an error been encountered?
    je else_l_7
    ...
    jmp short endif_l_7
    else_l_7:					;AN000; Otherwise, if no error.
	 ...
	 jnc @F					;AN000; Error closing the file?
	      MOV  N_WRITE_ERR_CODE, AX 	;AN000; Yes! Save the error code
	 @@:	 				;an000;
    endif_l_7:					;an000;
						;
    mov  ax, n_write_err_code			;an000; return the error code
    cmp n_write_err_code,0			;an000; have errors been encountered?
    je else_l_8
...
EXEC_PROGRAM_ROUTINE  PROC FAR			;AN000;
    ...
    POP  ES					;AN000; Point ES to the current data segment
						;
    cmp WORD PTR EX_RE_DIR,EXEC_DIR		;AN000;
    jne @F
...
;RESTORE REGISTERS				;
    ...
    CALL FAR PTR HOOK_INT_24			;AN000;
						;
    cmp WORD PTR EX_RE_DIR,EXEC_DIR		;AN000; Redirect Stdout?
    jne @F
	 MOV  AH, 3EH				;AN000; Close Stdout.
	 MOV  BX, 1				;AN000;
	 DOSCALL				;AN000;
    ...
    DOSCALL					;AN000;
    cmp AX,0					;AN000; SAR;If not zero...
    jne @F
    cmp EXEC_ERR,TRUE				;AN000; SAR
    jne else_l_10				;AN000; SAR
    @@:
...
GET_CNTY_DEF_ROUTINE	PROC FAR		;AN000;
    ...
    MOV  SI, AX 				;AN000; Move the address into an index reg
						;
    cmp BX,1					;AN000; BX contains which table to search
    jne else_l_11
...
GET_CNTY_INDEX_ROUTINE	PROC FAR		;AN000;
    ...
    while_l_4:
    cmp DX,2					;AN000; Search the TWO tables
    jg endwhile_l_4
	 cmp DX,1				;AN000; Are we searching the first table?
	 jne else_l_12
	 ...
	 while_l_3:
	 cmp WORD PTR [SI],CX			;AN000; Search until this code is found
	 je endwhile_l_3
	 cmp BX,AX				;AN000; And while there are still table entries
	 jg endwhile_l_3
	 ...
	 endwhile_l_3:
						;
	 cmp BX,AX				;AN000; Index is finished for this table
	 jle endwhile_l_4
	      INC    DX 			;AN000; Examine the next table
	 jmp while_l_4
	 else_l_13:				;AN000;
	      jmp short endwhile_l_4		;AN000; Exit the while loop
	 endif_l_13: 				;AN000;
						;
    jmp while_l_4				;AN000;
    endwhile_l_4:
...
GET_KYBD_INDEX_ROUTINE	PROC FAR		;AN000;
    ...
    jg endwhile_l_6
	 ...
	 else_l_15:				;AN000;
	    ...
	    jmp short endwhile_l_6		;AN000; Exit the while loop
	 endif_l_15: 				;AN000;
						;
    jmp while_l_6				;AN000;
    endwhile_l_6:
...
GET_KYBD_ROUTINE   PROC FAR			;AN000;
    ...
    MOV  SI, AX 				;AN000; Move the address into an index reg
    cmp CX,1					;AN000; CX Contains which table to search
    jne else_l_16
    ...
    else_l_20:					;AN000;
	 ...
	 @@:
	     ...
	     repeat_l_1:			;AN000;
		  ...
		  IN	  AL,DX 		;AN000; 				   @RH2
						;
		  cmp AX,XMAA_CARD_ID		;AN000;
		  je @F
		  cmp AX,HLST_CARD_ID		;AN000;JW
		  jne endif_l_17
		  @@:
		       MOV  SI, 1		;AN000; Yes! Expanded memory supported
		       jmp short endrepeat_l_1	;AN000;
		  endif_l_17:			;AN000;
						;
		  cmp CL,NUM_OF_SLOTS		;AN000;
		  jne @F
...
GET_PRINTER_PARAMS_ROUTINE   PROC FAR		;AN000;
    ...
    je else_l_22
	 ...
	 MUL  BX				;AN000;
	 MOV  SI, AX				;AN000;
	 cmp byte ptr PRINTER_TABLES[SI].PRINTER_DATA_VALID,1 ;AN000;
	 jne else_l_21
	      JMP  COPY_INFO			;AN000;
    ...
    else_l_22:					;AN000;
	 ...
	 while_l_7:
	 cmp bx,7				;an000; search the seven structures
	 ja endwhile_l_7
	      cmp printer_tables[si].printer_tab_num,ax   ;an000; does the printer number match?
	      jne @F
	      cmp byte ptr printer_tables[si].printer_data_valid,1 ;an000; and is this data valid?
	      jne @F
		   jmp	copy_info		;an000;
	      @@:				;an000;
...
SAVE_PRINTER_PARAMS_ROUTINE  PROC FAR		;AN000;
    ...
    cmp N_PRINTER_TYPE,'S'                      ;AN000; Is this a serial port?
    jne @F
    ...
    MOV  byte ptr PRINTER_TABLES[BX].PRINTER_DATA_VALID, 1 ;AN000; Indicate that the data is valid
    ...
    cmp CX,7					;AN000;
    ja endwhile_l_8
	 cmp PRINTER_TABLES[SI].PRINTER_TAB_NUM,AX ;AN000;
	 jne @F
	 cmp SI,BX				;AN000;
	 je @F
	      MOV  byte ptr PRINTER_TABLES[SI].PRINTER_DATA_VALID, 0 ;AN000;
	 @@:	 				;AN000;
...
HOOK_INT_24   PROC FAR				;AN000;
    ...
    cmp INT24_STATUS,UNHOOKED			;AN000;
    jne @F
