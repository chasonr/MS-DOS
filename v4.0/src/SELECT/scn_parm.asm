...
NEXT_PARSE:					 ;AN000;
    ...
    POP  DS					 ;AN000; Restore the data segment
						 ;
    cmp AX,0					 ;AN000; Were there any errors?
    jne elseif_l_1
    ...
    jmp short endif_l_1
    elseif_l_1:
    cmp AX,-1					 ;AN000; If there was an error, indicate so.
    je endif_l_1
...
SCAN_INFO_FILE_ROUTINE	PROC	FAR		 ;AN000;
    ...
    PUSH ES					 ;AN000; Save the extra register
						 ;
    cmp MODE,E_SELECT_INV			 ;AN000; If no error has occured yet,
    je endif_l_4
    cmp MODE,1					 ;AN000;   and if the parameter was FDISK...
    jne endif_l_4
	 ...
	 jnc @F					 ;AN000; If there was as error, do not continue
	 ...
	 jnc @F					 ;AN000; If there was an error, do not continue
	      JMP  FILE_ERROR			 ;AN000; Return INVALID parameters to the caller
	 @@:	 				 ;AN000;
	 ...
	 while_l_1:
	 cmp AX,0				 ;AN000; Continue while there are characters left
	 jle endwhile_l_1
	      MOV  DL, [SI]			 ;AN000; Get the character from the buffer
	      cmp DL,E_CR			 ;AN000; See if this is the end of the line
	      je @F
	      cmp DL,E_LF			 ;AN000;
	      jne endif_l_2
	      @@:
		   MOV	[BX], CX		 ;AN000; Store the length of path
		   jmp short endwhile_l_1	 ;AN000;
	      endif_l_2:			 ;AN000;
	      ...
	      cmp CX,M_INSTALL_PATH		 ;AN000; If more then 40 characters then an error
	      jbe while_l_1
		   JMP	FILE_ERROR		 ;AN000;
	 jmp while_l_1				 ;AN000;
	 endwhile_l_1:
	 cmp OPTION_1,1				 ;AN000; If option is 1 or 2 then a path is needed
	 jb else_l_3
	 cmp OPTION_1,3				 ;AN000;
	 ja else_l_3
	      cmp WORD PTR [BX],0		 ;AN000; If the path length was 0, return error
	      jne endif_l_3
		   JMP	FILE_ERROR		 ;AN000;
	 jmp short endif_l_3			 ;AN000;
	 else_l_3:				 ;AN000;
	      cmp WORD PTR [BX],0		 ;AN000; If option is 3 and there is a path, error
	      je @F
		   JMP	FILE_ERROR		 ;AN000;
	      @@:				 ;AN000;
	 endif_l_3: 				 ;AN000;
