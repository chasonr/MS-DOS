...
# Convert an EXE file to a COM file
# Unlike exe2bin, make-com.rb allows the input file to have relocations
# Replaces CONVERT.EXE
...
if !File.file?(tail_bin) || (File.mtime(tail_bin) < File.mtime(tail_src)) then
    system("jwasm -q -bin -Fo#{tail_bin} #{tail_src}")
    exit $? if $? != 0
end
